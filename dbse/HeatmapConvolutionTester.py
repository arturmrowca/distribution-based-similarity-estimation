...
import numpy
from dbse.tools.iron_tool_belt import *
...
class HeatmapConvolutionTester(BaseNode):
        ...
        for object_id in list(test_df["id"].unique()):
            ...
            for cluster_id in list(cur_df1["cluster_id"].unique()):
                ...
                for i in range(len(cur_df3)):
                    ...
                    predicted_risk, predicted_rul, m, all_feature_sum, per_feature_sum, feature_favorites = self._predict_RUL(data_in, current_test_df, cluster_id, all_feature_sum, None, timestamp_gap, expected_rul)
                    ...
                    # 6. wenn mehr als 2 features kleiner 0.53 sind dann nehme average dieser
                    ...
                    # 7. result should be at location of test_df WHERE current_test_df["RUL"].min()
                    ...
                    # 8. store last Timestamp for shifting if it is more urgent
    ...
    def _do_whole_model_prediction(self, cur_test_df, grid_area, dimensions, m, all_feature_sum, features_to_skip, timestamp_gap, dev_dict_path):
        ...
        for col in cur_test_df.columns:
            ...
            try:
                ...
                # 5. Variante 4: Average of current heats without outliers
                found_risk = avg
    ...
    def _visualize_summed_curve_m(self, xi, all_feature_sum):
        ...
        if self._enable_all_print: print("All Features: ")
    ...
    def get_cluster_values(self, cur_df1, cluster_id):
        ...
        # TEST - gotten by feature selection
        skip_features = {}
    ...
    def get_skip_feats(self, feature_skipping, skip_features, cluster_id):
        ...
        # run tester for this data frame and add column predicted
    ...
    def whole_model_prediction(self, cur_df1,  expected_rul, predicted_risk, predicted_rul, all_feature_sum, timestamp_gap, data, all_feature_favorites, only_cluster_id, only_object_id, last_ts, dev_dict_path1, test_mode = False):
        ...
        # use the more pessimistic estimate
        two_max = numpy.array([fst, scd, thr]).argsort()[-2:][::-1].tolist()
        model = sorted(two_max)[-1]
        fine_indices = [model]

        # Pessimistic is in the middle, and the sum of integrals is largest in the middle?then the probability is at the right corner.
        # Because of PHM, try to be as pessimistic as possible.

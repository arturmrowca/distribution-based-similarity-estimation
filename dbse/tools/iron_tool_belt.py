...
def find_n_grams(n, single_sequence):
    ...
    grams = [single_sequence[i:i+n] for i in range(len(single_sequence)-n)]

    # Suppose 'grams' is your array/list of items
    import numpy as np
    unique_grams, counts = np.unique(grams, return_counts=True)
    # If you want a 2D array similar to the old 'itemfreq' output:
    freqs = np.column_stack((unique_grams, counts))

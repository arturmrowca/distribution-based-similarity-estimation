...
import traceback
import pandas as pd
from dbse.tools.parallel import parallelize_stuff
import numpy


class HeatmapConvolutionTrainer(object):
    '''
    Attention:  One model is defined per cluster id! Make sure to include a cluster_id in the data.
    ...
    def store_model(self, trained_model, model_file):
        """
        ...
        print("Stored Model to %s" % str(model_file))
    ...
    def run(self, data_in, extract_frame_override = False, train_dff = None):
        ...
        for cluster_id in list(train_df["train_cluster_id"].unique()): # per cluster own model
            ...
            # 2.1. scale data and remove outliers
            ...
            # 2.3. Store model for this cluster
            data_in["CL_" + str(cluster_id) + "_" + self._field_out_train_model] = model_per_feature

        # 3. Transform to output model
        model = {key: data_in[key] for key in data_in.keys() if str.startswith(key, "CL_") or key in ["train_risc", "train_rul"]}
    ...
    def _remove_one_outlier(self, feature_df, col_name, nr_iterations =3, std_threshold=1):  
        ...
        # 2. iterate multiple times over window
    ...
    def _build_one_heat_map(self, feature_df, risk_min, feature_min, feature_max, fine_tune = -1):
        ...
        # 1. Apply model learning on whole input
        if fine_tune == -1:
            try:
                ...
                # Assign X Y Z
                X = feature_df.RISK.values
                Y = feature_df[feature_df.columns[1]].values
                Z = values
                ...
                for r in range(len(grad)):
                    ...
                    if list(nonzeros): 
                        ...
                        # fill from nonzeros[-1] to len(grid)-1
            ...
            # 2.1. Apply model learning on subset of input
            if fine_tune == 0:
            ...
            try:
                ...
                # Smooth with a Gaussian kernel 
                kernel = Gaussian2DKernel(x_stddev=self._std_gaus, y_stddev=self._std_gaus, x_size=self._kernel_size, y_size=self._kernel_size)
                grad = scipy_convolve(grid_cur, kernel, mode='same', method='direct')
            
                # vertical interpolation up to edges
                for r in range(len(grad)):
                    # per dimension get first and last nonzero value
                    cur_line = grad[:, r]
                    nonzeros = numpy.where(cur_line > 0.0001)[0]
    ...
    def _build_heat_map_parallel(self, output_dfs):
        """
        Parallelization method to use all CPUs for training
        """

        # output ist dimensions
        dimensions = {}
        print("Building " + str(len(output_dfs)) + " heat maps")
        
        # 1. create input arguments for each run
        ...
        # 2. Determine maximum number of available CPUs
        ...
        # 4. Return training result and plot if specified
        for res in res_list:
            name = res[0]
            dim = res[1]
            grid_cur = res[2]
            ...
            if True and self._visualize_heatmap:
                ...
                try:
                    ...
                    ax_mag.imshow(dim[0][::-1,::-1], cmap='RdYlGn')
    ...
    def _outlier_removal(self, train_df, remove_empty, nr_iterations, split_windows, std_threshold):
        ...
        for col in train_df.columns:
            ...
            # 4. Merge result to common dataframe
    ...
    def _scale(self, train_df):
        ...
        for col in train_df.columns:
            ...
            try:
                scaler = scaler.fit(train_df[col].values.reshape(-1,1))
            except:
                scaler.fit(train_df[col].reshape(-1,1))
            try:
                train_df['scaled_'+col] = scaler.transform(train_df[col])
            except:
                train_df['scaled_'+col] = scaler.transform(train_df[col].values.reshape(-1,1))
